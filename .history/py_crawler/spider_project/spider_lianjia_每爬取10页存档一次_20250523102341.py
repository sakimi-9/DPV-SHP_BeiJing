    ...
    async def parse(self, text):
        ...
        for li in lis:
            try:
                ...
                attention_num_raw = li.xpath('.//div[@class="followInfo"]/text()')[
                    0
                ]  # 关注人数和发布时间原始文本
                ...
                property_type = house_info_parts[0] if len(house_info_parts) > 0 else ""
                area_str = house_info_parts[1] if len(house_info_parts) > 1 else ""
                floor_info = house_info_parts[2] if len(house_info_parts) > 2 else ""
                orientation = house_info_parts[3] if len(house_info_parts) > 3 else ""
                building_year_info = (
                    house_info_parts[4] if len(house_info_parts) > 4 else ""
                )
                ...
                # 简单提取建筑年份数字
                year_match = re.search(r"(\d{4})年建", building_year_info)
                building_year = year_match.group(1) if year_match else ""

                # 解析关注人数和发布时间
                attention_parts = attention_num_raw.split("人关注 / ")
                attention_count_str = (
                    attention_parts[0] if len(attention_parts) > 0 else ""
                )
                post_time_text = attention_parts[1] if len(attention_parts) > 1 else ""

                # 简单提取关注人数数字
                attention_count_match = re.search(r"(\d+)", attention_count_str)
                attention_count = (
                    attention_count_match.group(1) if attention_count_match else ""
                )
...
if __name__ == "__main__":
    spider = Spider()
    logging.info("爬虫启动")
    asyncio.run(spider.main(start_page=1, stop_page=5))  # 设置爬取页数为1到5，
