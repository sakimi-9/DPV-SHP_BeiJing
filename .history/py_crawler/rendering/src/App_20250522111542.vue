...
const setupThreeJSScene = (container: HTMLElement, data: ParsedHouse[]) => {
  ...
  controls.keyPanSpeed = 7.0; // Optional: Adjust keyboard pan speed

  // Default mouse buttons: LEFT for rotate, MIDDLE for zoom, RIGHT for pan
  // We will rely on keyboard for pan now.
  // If you want to disable mouse pan: controls.mouseButtons = { LEFT: THREE.MOUSE.ROTATE, MIDDLE: THREE.MOUSE.DOLLY };

  // 5. Add a simple object (Cube) - Replace with actual data visualization later
  ...
  const animate = () => {
    ...
    if (cube) {
      cube.rotation.x += 0.00; // Stop automatic rotation for manual control
  ...
  // Store the contextmenu handler if you keep preventing it
  ...
  console.log('Three.js scene setup complete with keyboard controls for pan.');
};
...
const cleanupThreeJSScene = () => {
  ...
  if (scene) {
    ...
    // A simple way to dispose children: scene.children.forEach(child => scene.remove(child));
