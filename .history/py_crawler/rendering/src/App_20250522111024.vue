...
const setupThreeJSScene = (container: HTMLElement, data: ParsedHouse[]) => {
  ...
  // No change needed here if we just prevent context menu
  ...
  (renderer.domElement as any).__contextMenuHandler = (event: MouseEvent) => event.preventDefault();
  renderer.domElement.addEventListener('contextmenu', (renderer.domElement as any).__contextMenuHandler);


  console.log('Three.js scene setup complete with OrbitControls and context menu prevented.');
};
