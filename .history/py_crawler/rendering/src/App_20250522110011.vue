...
watch(currentMode, (newMode, oldMode) => {
  ...
  if (newMode === '3D') {
    ...
    if (threeContainer.value) {
      console.log('Attempting to setup 3D scene with container:', threeContainer.value); // Added log
      // TODO: Pass actual parsed data to setupThreeJSScene
      setupThreeJSScene(threeContainer.value as HTMLElement, parsedHouseData);
    } else {
      console.warn('threeContainer.value is null when attempting to switch to 3D mode.'); // Added log
    }
  } else if (newMode === '2D') {
