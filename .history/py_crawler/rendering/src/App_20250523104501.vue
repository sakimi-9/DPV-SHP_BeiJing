...
function parseHouseData(rawData: any[], areaCoords: AreaCoordinates): ParsedHouse[] {
  ...
  rawData.forEach(item => {
    const parsedItem: ParsedHouse = {
      ...
      单价: null,
      关注人数: null,
      发布时间: item && item['关注人数和发布时间'] ? item['关注人数和发布时间'].split(' / ')[1]?.trim() || null : null,
    ...
    const followTimeParts = item && item['关注人数和发布时间'] ? item['关注人数和发布时间'].split(' / ').map((part: string) => part.trim()) : [];
...
onMounted(async () => {
  try {
    ...
    houseData.value = []; // Set to empty array on error to prevent infinite loading state
  }
});
