...
// 绘制单价分布直方图
function renderPriceHistogram(data: ParsedHouse[]) {
  if (!priceHistContainer.value) return;
  ...
  // 定义直方图的 bin 数量或间隔
  const binCount = 20; // 示例：分成20个 bin

  // 使用 Echarts 的 histogram 工具计算 bin 数据
  const histogramData = echarts.dataTool.histogram(prices, binCount);

  const myChart = echarts.init(priceHistContainer.value);
  const option = {
    title: {
      text: '单价分布',
      left: 'center'
    ...
    xAxis: [
      {
        type: 'category',
        data: histogramData.map(item => item[0]), // bin 的范围作为 x 轴标签
    ...
    series: [
      {
        name: '房源数量',
        type: 'bar',
        data: histogramData.map(item => item[1]), // bin 中的数据数量作为 y 轴值
      }
    ]
  };
  myChart.setOption(option);
...
// 绘制面积分布直方图
function renderAreaHistogram(data: ParsedHouse[]) {
  if (!areaHistContainer.value) return;

  // 过滤掉没有面积的数据，并提取面积数组
  const areas = data
    .map(item => item.面积)
    .filter((area): area is number => area !== null && !isNaN(area));

  // 定义直方图的 bin 数量或间隔
  const binCount = 20; // 示例：分成20个 bin

  // 使用 Echarts 的 histogram 工具计算 bin 数据
  const histogramData = echarts.dataTool.histogram(areas, binCount);

  const myChart = echarts.init(areaHistContainer.value);
  const option = {
    title: {
      text: '面积分布',
      left: 'center'
...
onMounted(async () => {
  ...
  try {
    ...
    if (currentMode.value === '2D') {
      ...
      renderPriceHistogram(parsedHouseData);
      renderAreaHistogram(parsedHouseData);
