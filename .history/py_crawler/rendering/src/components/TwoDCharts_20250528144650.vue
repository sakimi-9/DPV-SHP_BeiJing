<template>
    <div class="charts-2d-container">
        ...
        <div class="chart-carousel">
            <button @click="prevChart" :disabled="currentIndex === 0" class="nav-button left-arrow">&lt;</button>
            <div class="chart-item-single">
                <h3>{{ currentChart.title }}</h3>
                <div :ref="currentChart.ref" style="width: 100%; height: 400px;"></div>
            </div>
            <button @click="nextChart" :disabled="currentIndex === charts.length - 1"
                class="nav-button right-arrow">&gt;</button>
        </div>
...
.charts-2d-container {
    ...
    gap: 20px;
    align-items: center;
    /* Center the chart carousel horizontally */
}
...
.chart-carousel {
    ...
    width: 80%;
    /* Adjust width to leave space for arrows */
    max-width: 1000px;
    /* Optional: maximum width for large screens */
    display: flex;
    align-items: center;
    justify-content: center;
    /* Remove padding here, we'll position arrows relative to the container */
}
...
.nav-button {
    ...
    /* Remove padding */
    margin: 0 10px;
    /* Add horizontal margin for spacing from the chart area */
}

.left-arrow {
    left: -40px;
    /* Position left arrow outside the container, adjust as needed */
}

.right-arrow {
    right: -40px;
    /* Position right arrow outside the container, adjust as needed */
}

.nav-button:disabled {
    color: #ccc;
    /* Grey out disabled arrows */
    cursor: not-allowed;
}

.chart-item-single {
    flex-grow: 1;
    /* Allow chart item to grow */
    min-width: 0;
    /* Allow shrinking below min-width */
    width: 100%;
    /* Take full width within carousel */
    box-sizing: border-box;
    /* Add padding to the chart item itself to match the image's inner spacing */
    padding: 20px;
    border: 1px solid red;
    /* Add red border as in the image */
}

.chart-item-single>div {
    width: 100%;
    height: 400px;
    /* Ensure the chart div has a height */
}
