...
function renderBubbleChart(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const option = {
        ...
        visualMap: [{ // Map关注人数 to bubble size
            min: 0,
            max: Math.max(...bubbleData.map(item => item.value[2])),
...
function renderParallelChart(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const option = {
        ...
        visualMap: { // Optional: map 单价 to color
            min: Math.min(...parallelData.map(item => item[4])),
            max: Math.max(...parallelData.map(item => item[4])),
...
function renderAreaPriceHeatmap(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const option = {
        ...
        visualMap: {
            min: 0,
            max: Math.max(...heatmapData.map(item => item[2])), // Max count
            calculable: true,
            orient: 'horizontal',
            left: 'center',
            bottom: '10%',
