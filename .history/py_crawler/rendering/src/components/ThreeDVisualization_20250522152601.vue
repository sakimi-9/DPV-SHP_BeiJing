...
function generateChartTexture(chartType: string, data: ParsedHouse[]): THREE.Texture | null {
    ...
    // Dispose of the canvas element if not needed after texture creation to free up memory
    // This might be tricky if echarts needs to keep the instance for interactivity,
    // but for static textures on a cube, disposing might be fine.
    // For now, let's keep the canvas for potential debugging, but in a production app,
    // you might want to consider if the chart instance needs to persist.
    // echarts.dispose(chart); // Dispose echarts instance if created internally
