...
// Import the necessary components and features
import { BarChart, PieChart, ScatterChart } from 'echarts/charts';
import { GridComponent, TooltipComponent, LegendComponent, TitleComponent } from 'echarts/components';
import { CanvasRenderer } from 'echarts/renderers';

// Import and register the DataTool feature
import { DataTool } from 'echarts/features';
...
echarts.use([
  ...
  TitleComponent,
  CanvasRenderer,
  DataTool // Register the DataTool
]);
...
const drawUnitPriceDistributionChart = (chartElement: HTMLElement, parsedData: ParsedHouse[]) => {
  ...
  // Use echarts.dataTool.histogram - now it should be available after registering DataTool
  const histogramData = echarts.dataTool.histogram(prices);
  ...
  const options = {
    title: { text: '单价分布' },
    tooltip: {},
    xAxis: { type: 'category', data: bins.map((bin: { interval: number[]; }) => `${bin.interval[0]}-${bin.interval[1]}`) }, // Add type annotation for bin
    yAxis: { type: 'value' },
    series: [{
...
const drawSizeDistributionChart = (chartElement: HTMLElement, parsedData: ParsedHouse[]) => {
  ...
  // Use echarts.dataTool.histogram - now it should be available after registering DataTool
  const histogramData = echarts.dataTool.histogram(sizes);
  const data = histogramData.data;
  const bins = histogramData.bins;
...
onMounted(async () => {
  try {
    ...
    if (currentMode.value === '2D') {
      ...
      if (areaChartContainer.value) renderAreaChart(parsedHouseData);
      ...
      if (unitTypeChartContainer.value) renderUnitTypeChart(parsedHouseData);
      if (followPriceScatterContainer.value) renderFollowPriceScatter(parsedHouseData);
      if (followAreaScatterContainer.value) renderFollowAreaScatter(parsedHouseData);
    }
